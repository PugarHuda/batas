...
The agent reads the position, derives its floor from measured price movement, compiles the program, checks its hash against the deployed router, ships, and publishes the grant to a Hedera Consensus Service topic. A second agent, agent/counterparty.mjs, finds the x402 service through /.well-known/x402, asks three free questions, and pays a metered price (0.0012 HBAR for this question) only to learn who operates the position and whether their ERC-8004 identity (#10123) vouches. Then it trades and writes 60bps-above-floor feedback to the ERC-8004 reputation registry, which refuses the agent's own owner.
...
Hedera: x402 through the Blocky402 facilitator, priced in HBAR first because an HTS token must be associated with an account before it can be received. A caller already associated with the project's HTS token, Batas Inspection Credit, can pay in it instead, and that settlement assesses the token's fixed custom fee on the ledger; a 1% fractional fee was tried first and assessed nothing, because Hedera does not charge a fractional fee on a credit to the fee collector. Note that the official Hedera x402 proof of concept points its testnet config at x402.org and reaches for Blocky402 only on mainnet; Blocky402 does serve hedera:testnet, at api.testnet.blocky402.com, whose /supported sits at the root rather than under /v1. Copy the PoC as-is and you settle through the wrong facilitator with everything appearing to work. Grants and revocations are both published to a Hedera Consensus Service topic, readable by anyone with no account, because a ledger carrying only grants is the optimistic half of the story.
...
| "Use one or more of … Hedera Agent Kit, OpenClaw ACP, x402, A2A protocol, or Hedera SDKs directly" | x402 through Blocky402 (`@x402/hedera`), the A2A protocol with the a2a-x402 extension, `@hiero-ledger/sdk` for HCS and HTS, and a Hedera Agent Kit plugin exported as `batas/hedera-agent-kit` offering all six tools, including `check_payment_trail`, which reads the HCS payment trail verified against the ledger; `agent/mcp.test.mjs` drives it through the kit's `HederaLangchainToolkit` |
...
| Extra point | Status |
|---|---|
| "On-chain agent identity using ERC-8004 or HCS-14" | **met** — agent #10123 in the identity registry, with an HCS-14 universal agent id published in its Hedera profile, and client feedback in the reputation registry (`60bps above the floor`), which the contract will not let the agent write about itself |
| "Pay-per-call inference, data, or compute metering rather than a flat per-request charge" | **met** — the 402 amount is computed from each request body: 40,000 tinybar to decode, 1,000 per instruction, 34,000 for the HCS publication lookup, 20,000 each for the ENSv2 authority and ERC-8004 operator reads. The paid answer itemises the bill in `metering`, and a payment signed for a light body is refused for a heavy one. The live mandate alone costs 0.001 HBAR; with the operator check, 0.0012 HBAR — settled against the deployed service in `0.0.7162784@1789305887.190425507`, whose audit record is topic message #17 |
| "Multi-agent negotiation and settlement via A2A or ACP" | **met** — `agent/a2a-client.mjs` negotiates a fill with the Batas agent over A2A JSON-RPC (`/a2a`). An oversized proposal is countered with the largest size that clears the live mandate, at its floor; the accepted terms are paid inside the same task under the a2a-x402 extension. Settled against the deployed service in `0.0.7162784@1789305149.263635025` |
| "Agent discovery via UCP, or a directory that makes your service findable by other agents" | **met** — `agent/counterparty.mjs` starts from the ERC-8004 identity registry, not a URL: the endpoint comes from agent #10123's registration, the payee and network from its on-chain metadata, and the host's `/.well-known/x402` must agree with both before anything is spent. On Hedera itself, Batas is listed in the Hashgraph Online HCS-10 registry (topic `0.0.6913983`, entry #384) with an HCS-11 profile naming the x402 endpoint, its metered price read from the live manifest (profile `0.0.10524249`), the A2A endpoint and agent.batas.eth; `npm run hol -- --find` finds it from the registry alone |
| "HTS tokens or custom fee schedules in the settlement path" | **met** — both. The paid route also accepts Batas Inspection Credit, HTS token `0.0.10523367`, settled by Blocky402; the token's fixed 0.01 BIC custom fee is assessed in the settlement itself — against the deployed service in `0.0.7162784@1789305187.730633008` (`assessed_custom_fees` on the mirror node) |
| "Verifiable payment audit trails on HCS" | **met** — after every settlement the payer publishes a `batas.payment` record to topic `0.0.10394165`: transaction id, payer, payee, amount, and keccak256 of the request and response bodies. `npm run payments` re-checks each record against the ledger and lists forged or mismatched ones as unverified. First record: message #16 for `0.0.7162784@1789303433.642299625` |
| "Recurring or streamed payments using Scheduled Transactions" | **met** — `agent/subscribe.mjs` creates a cancellable standing order of scheduled HBAR payments. Order 1789303558 ran by consensus at its due times as schedules `0.0.10523344`, `0.0.10523346`, `0.0.10523347`; a second order was deleted before it paid |

### 2. 1inch — Build an Aqua App ($5,000)

| Requirement | Where it is met |
|---|---|
| "a custom Aqua app that implements a sophisticated DeFi position … demonstrated through tests scripts or a UI" | a position whose size cap, floor and expiry are enforced inside settlement, shown by `script/Demo.s.sol` and the live instrument at https://batas-one.vercel.app/app; and a two-sided banded position (a PolicyEnvelope per side, liquidity concentrated in a band, a decaying spread against counter-trades) that is live on Sepolia. A separate maker shipped it to the real Aqua against the deployed router around the live pool's spot ([ship](https://sepolia.etherscan.io/tx/0x15b6bf658d95ed853a668d03ea7bc6f4fef00f9ae4cfadce7b40da77577389c9)) and traded it once each way ([B→A](https://sepolia.etherscan.io/tx/0x69eda36bdaaf01112f9e92d8163d67f6a4ce629eb97a64f252cecf4c7175d4e6), [A→B](https://sepolia.etherscan.io/tx/0x8da6253af8ac55fca6ebd5c8daef4c0f31490d271cf20b4d8982a167b6b0b94c)). Over-cap and under-floor trades are refused by eth_call with the envelope's own errors. `npm run band` and `agent/band-live.test.mjs` read it back from the chain and match every swap's Transfer logs to the priced amount to the wei, and `npm run test:band` proves the same mechanics on a fork. The decoder is cross-checked against 1inch's official `@1inch/swap-vm-sdk` (`npm run test:sdk`) |
...
|---|---|
| "deploy your own subname registry to tokenize and manage subnames under your own rules" | **met** — the registry above holds the agent's authority, and is the subregistry of `batas.eth`, registered on ENSv2 through the registrar's commit and reveal ([register](https://sepolia.etherscan.io/tx/0xe01d01d37a649db0a3573ad435913bf9530a594f5789d792d2ac22cee7dcb12f)) |
...
| "Enhanced Access Control … to delegate specific rights" | **met** — beyond the roles above, the counterparty account holds `ROLE_SET_TEXT` on one key of `agent.batas.eth` and nothing else: [grant](https://sepolia.etherscan.io/tx/0x3da4cc3543bad065175d920bdb63ca5deb6e1c54b8e7d78b7f5ac68a03f0e1cc), its own [write](https://sepolia.etherscan.io/tx/0x09ab95bb180c7e7df8e5419c340b8ac5e23714522536e60647c23de3600276d2); the resolver refuses it `url`, the endpoint records, and the same key on `batas.eth` |
| "Give subnames their own Permissioned Resolver" | **met** — `agent.batas.eth` points at a PermissionedResolver deployed through ENS's VerifiableFactory, [`0x671C506A…`](https://sepolia.etherscan.io/address/0x671C506Aaa2a123bE802Fe51975Ca9515AEC2516), holding its records under name-scoped roles. The project owns it; `batas.eth` shares it |
| "resolve subnames straight off a parent's resolver with wildcard resolution" | **met** — a label registered nowhere under `batas.eth` resolves through the UniversalResolver to the parent's resolver (`npm run ens:resolve`, `agent/ens-hierarchy.test.mjs`) |
| "record aliasing at the resolver level or namespace aliasing via a shared registry" | **met** — record aliasing at the resolver: `mandate.batas.eth` is registered nowhere and answers with `agent.batas.eth`'s address and records ([multicall](https://sepolia.etherscan.io/tx/0x9d141f7051223969d48f49fce404b785780d21a7d1f4d6ba7778b5ec58eaa413)) |
| "agents as namespaces, each with their own identity and permissions" (bonus) | **met** for both agents Batas runs. `agent.batas.eth` is the maker's agent, ERC-8004 #10123, whose registration names it back ([uri](https://sepolia.etherscan.io/tx/0x224e89f82d0cddf795f19f80abf7b33753ebfb7401bcccc8fbcde0653662d43c)). `counterparty.batas.eth` is the agent in `agent/counterparty.mjs`, which trades, pays for inspections and writes reputation. It was granted by the maker, expiring and soulbound with `ROLE_CAN_TRANSFER_ADMIN` withheld, and the grantor keeps UNREGISTER/RENEW ([register](https://sepolia.etherscan.io/tx/0x439046099313a7a31a0c43901cefac293a080c8532023ec13fbddda6351eb18d)). Its only resolver permission is `ROLE_SET_TEXT` on its own name ([grant](https://sepolia.etherscan.io/tx/0xb083c087f8d19c80df29af6aa30a49a0136dbf25e9b018d6488d9a200fa62720)). With its own key it minted ERC-8004 #10258 ([register](https://sepolia.etherscan.io/tx/0x967a2895a649c237711b0f005ddadafaf715b6bb187c961439d08378043df081)) and wrote its own ENSIP-26 and ENSIP-25 records ([records](https://sepolia.etherscan.io/tx/0x2a7035fffb1a9ba0ac6d68a1ae13e4f52125ed10073c303c6cd4678c61f946b6)). The resolver refuses it on agent.batas.eth's records (`EACUnauthorizedAccountRoles`, pinned in `agent/namespaces.test.mjs`). `npm run namespaces` shows both names linked both ways. One link is missing and said so: the mandate registry was created without `ROLE_SET_PARENT`, so it cannot name `batas.eth` as its parent; resolution walks down from the root and does not need it |
