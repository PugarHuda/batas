...
| 4 | 0:38–0:58 | `src/PolicyEnvelope.sol`, `exec()`: cursor on `ctx.runLoop()`, then the error lines | **Mandate hash = strategy hash → Guard wraps the program. 933 gas.** | Batas makes the limits the position's identity. The encoded mandate is the strategy bytes Aqua hashes, so a program with the limits removed is a different position nobody funded. And this is a new SwapVM instruction. It runs the rest of the program inside itself and checks the settled amounts on the way out. Placed first, nothing can undo it. It costs nine hundred and thirty-three gas. |
...
3. *What does the guard cost?* 933 gas warmed, about 0.8% of a settlement, measured by
