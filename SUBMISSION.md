...
| "Use one or more of … Hedera Agent Kit, OpenClaw ACP, x402, A2A protocol, or Hedera SDKs directly" | x402 through Blocky402 (`@x402/hedera`), the A2A protocol with the a2a-x402 extension, `@hiero-ledger/sdk` for HCS and HTS, and a Hedera Agent Kit adapter exported as `batas/hedera-agent-kit` |
...
|---|---|
| "a custom Aqua app that implements a sophisticated DeFi position … demonstrated through tests scripts or a UI" | a position whose size cap, floor and expiry are enforced inside settlement, shown by `script/Demo.s.sol` and the live instrument at https://batas-one.vercel.app/app; and a two-sided banded position — per-side PolicyEnvelope, liquidity concentrated between 1.8 and 2.2, a decaying spread against counter-trades — which `npm run test:band` ships to the real Aqua on a Sepolia fork against the deployed router, trading both ways with every balance matching the priced amount to the wei. The decoder is cross-checked against 1inch's official `@1inch/swap-vm-sdk` (`npm run test:sdk`) |
...
| "agents as namespaces, each with their own identity and permissions" (bonus) | **met** for the one agent this project runs — `agent.batas.eth` carries ENSIP-26 agent records and an ENSIP-25 `agent-registration` record for ERC-8004 agent #10123, whose registration names `agent.batas.eth` back ([uri](https://sepolia.etherscan.io/tx/0x224e89f82d0cddf795f19f80abf7b33753ebfb7401bcccc8fbcde0653662d43c)); `npm run ens:verify` accepts the link only when both halves agree. Its permissions are the soulbound role bitmap. One link is missing and said so: the mandate registry was created without `ROLE_SET_PARENT`, so it cannot name `batas.eth` as its parent; resolution walks down from the root and does not need it |
